...
> **✅ P2′ STATUS (closed 9 Jul — ~85%; scorecard in chat/DECISIONS):** DONE: orchestrator generalized + fresh-clone auto-build · SG 9 acts/9,078u · MY 8/851 (OCR live, CER-proxy 0.986) · AU 9/9,402 (browserless API route, multi-volume) · **⭐ MY error-audit: 124 findings/5 classes** · cost meter → logs/cost_report.json · curation layer (candidate/legal_review/final) · NEW/KNOWN cross-economy · consolidated.csv (67 rows). **CARRIED into P3′ (verification depth + P7 recall):** gates G2/G5/G6/G8 + dangling-ref · AU P7 gold recall 0% (15 RECALL HOLEs logged) + MY P7 review · JSON envelope fields (archived_copy / ocr_quality_cer / reviewer_decision per row) · Path A (Ollama) never verified · archive.org dead-link fallback · Schedule parsing (may stay a Known Limitation). CONSCIOUSLY DROPPED: federalgazette.agc.gov.my connector (seeds + lom.agc.gov.my covered all MY gold — record in Known Limitations). No cross-encoder rerank (nano screen plays that role — Known Limitations).

### P3′ — Differentiators + CORE FREEZE · **Jul 13–16** · *"the points other teams miss"* — **AMENDED 9 Jul (execution workstreams A–F; supersedes the bullets below where they differ)**

> **A (Jul 10–11) Recall repair first** (reviewer rule): AU P7 hole-closure (≥60% provision recall), MY Codes-of-Practice load, numpy dense-scan (~10×), full re-runs. **A5:** amendment/"Last Amended" detection + dead-link→archive.org fallback feeding G4/`Status`.
> **B (Jul 11–13) NEW-recall max + FP control:** all-pillar seed sweep, hunt-list fetch, counter-retrieval for exceptions, adversarial false-NEW kill pass (3 refuter personas + DoDont §13 ❌-bank) + YOUR sign-off per NEW row.
> **C (Jul 12–14) Gate stack completion:** G2 location · G5 whole-rule · G6 meaning-support · G8 counter-evidence + dangling-reference · citation tiers `[settled]/[verify]/[verify-pinpoint]` · envelope fields (archived_copy, ocr_quality_cer, access_date, **reviewer_decision**).
> **D (Jul 13–15) Graph enrichment** (GraphRAG §10 win condition, deterministic edges only): CROSS_REFERENCES from regex · MAPS_TO/EVIDENCED_BY/KNOWN_AS write-back for gate-passing rows · saved Cypher demo queries.
> **E (Jul 14–15) Zone-3 noise audit:** deterministic §9.1 scores (polarity, court-order, gov-data exclusion) → multi-persona judges → Krippendorff's α → uncertainty band; AI-suggests-YOU-approve.
> **F (continuous → 🔒 Jul 16) Freeze mechanics:** daily legal_review.csv pass (corrections → DoDont §13 → regression tests) · §13 ✅/❌ bank as pytest cases · **F2: pitch policy-narrative draft by Jul 15** · **freeze gate: one full Path A (CPU-only/key-free/SQLite/Ollama) run must pass** · full 3-economy sweep + consolidated + cost report + git tag `core-freeze`. Cut order if slipping: D-demo-queries → E-polish → B2 — never A, C, B4.
