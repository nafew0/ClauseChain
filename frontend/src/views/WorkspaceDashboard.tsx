...
export default function WorkspaceDashboard() {
  ...
  return <WorkspaceShell breadcrumbs={[{ label: 'Dashboard' }]}><div className="cc-page live-dashboard">
    ...
    {query.isError || !data ? <PageUnavailable title={query.isPending ? 'Loading the authoritative workspace…' : 'Dashboard data is unavailable'} /> : <>
      ...
      <section className="dashboard-champion" data-data-card><div className={data.champion.status === 'PASS' ? 'pass' : 'fail'}>{data.champion.status === 'PASS' ? <CheckCircle2 /> : <ShieldAlert />}<div><span>Release readiness</span><strong>{readinessLabel(data.champion.status)}</strong></div></div><ul>{Array.isArray(data.champion.failures) && data.champion.failures.length ? data.champion.failures.map((failure, index) => <li key={index}>{friendlyFailure(failure)}</li>) : <li>All release checks are green.</li>}</ul></section>
