...
export default function SubmissionExplorer() {
  ...
  return <LazyMotion features={domAnimation}><MotionConfig reducedMotion="user"><div className="submission-explorer"><header className="submission-header"><div><span><FileCheck2 size={14} /> Candidate evidence + verification</span><h1>Submission explorer</h1><p>Final artifacts are produced only by deterministic engine replay—not by this table.</p>{query.data?.release ? <em className="submission-release-state">Release {query.data.release.state}</em> : null}</div>{user?.is_superuser ? <button onClick={replay} disabled={launch.isPending || ['queued', 'running'].includes(latestReplay?.status ?? '')}><Play size={15} /> Run approval replay</button> : null}</header>
